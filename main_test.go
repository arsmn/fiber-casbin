...
func Test_ConstructRequiresPermission(t *testing.T) {
	tests := []struct {
		name        string
	...
	for _, tt := range tests {
		app := *fiber.New()
		authz := NewWithConstructEnforcer(ConstructEnforcerConfig{
...
func Test_ConstructRequiresRoles(t *testing.T) {
	tests := []struct {
		name       string
		lookup     func(*fiber.Ctx) string
		roles      []string
...
func Test_RequiresPermission(t *testing.T) {
	...
	for _, tt := range tests {
		...
		authz := NewWithEnforcer(e, Config{
			Lookup: tt.lookup,
		})
